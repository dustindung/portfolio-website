...
const Header = () => {
  return (
    <header className="z-[1] relative">
      ...
      <nav
        className="flex fixed top-[0.15rem] left-1/2 -translate-x-1/2 h-[3rem] py-2
      ...
      >
        ...
        >
          {links.map((link) => (
            ...
            >
              <Link
                className="flex w-full items-center justify-center p-3 hover:text-gray-950 transition-colors"
                href={link.hash}
              >
                {link.name}
              </Link>
